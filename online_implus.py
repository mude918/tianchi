...
f = []
wf = []
swf = open('tianchi_mobile_recommendation_predict.csv', 'w')
swf.write('user_id,item_id\n')
for idx in range(block):
	path2.append(r'data_%s_%s.csv' %(sample_size, idx))
	path3.append(r'online_ans_%s_%s.csv' %(sample_size, idx))
	...
	print 'training item number:\t', len(offline_candidate_day30)
	print '---------------------\n'
	print 'online candidate item number:\t', len(online_candidate_day31)
	print '---------------------\n'
	...
	#get train x,y
	x = np.zeros((len(offline_candidate_day30), 4))
	y = np.zeros((len(offline_candidate_day30)), )
	id = 0
	...
	px = np.zeros((len(online_candidate_day31), 4))
	id = 0
	...
	### combine
	lx = zip(online_candidate_day31, py)
	#print lx
	print '---------------------'
